...
// is adoption data from public registries, never part of the Overall score, so it stays small,
    ...
    <span className="flex flex-wrap items-center gap-x-3 gap-y-1">
      {drawable.map(({ label, points }) => {
        ...
        return (
          <span
            key={label}
            title={`${label}: ${formatCompact(first.value)} on ${shortDate(first.date)} → ${formatCompact(last.value)} on ${shortDate(last.date)} — public registry snapshots, not part of the Overall score.`}
