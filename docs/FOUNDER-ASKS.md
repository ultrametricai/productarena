...
| API-score pill makes the header's API ✓ glyph redundant — drop the glyph when the score renders | in-flight (orchestrator) |
| Homepage tagline → "Open rankings for the AI era" (h1 + OG card og4.png + alt text) | in-flight (orchestrator) |
| Homepage title moves ABOVE the Companies\|Processes tabs | in-flight (orchestrator) |
| Homepage 'Products' as 3rd mode tab, replacing the include-all-products checkbox (legacy ?all=1 honored) | in-flight (orchestrator) |
| Evidence column at the end of the rankings tables (homepage mega-table + arena tables) → /score receipt | in-flight (orchestrator) |
| Supabase into serverless-databases (developer databases), judged vs neon | in-lane (supabase-dbs) |
| Drop the 'commercial' tag on product pages — they're (almost) all commercial | in-flight (orchestrator) |
| 'Technologies' ranking set: abstract control surfaces (API, CLI, MCP, terminal vs app…) as a 'Control surfaces' arena with toggles like 'Most mobile' / 'Most visual', AI/agentic-focused | open (needs design — abstract technologies, not products; evidence model TBD) |
