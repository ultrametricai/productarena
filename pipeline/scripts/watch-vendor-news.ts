...
// lexical signal for FOCUS, not a verdict input: it never touches any Overall score.
//
// Invocation:
...
export async function run(argv: string[]): Promise<NewsState> {
  ...
  const state: NewsState = {
    _comment:
      'Generated by pipeline/scripts/watch-vendor-news.ts — recent blog/changelog post titles per tracked product, agentic-relevant ones keyword-flagged (a FOCUS signal for the unlinked /ops dashboard, never a Overall score input). Budgeted and resumable: budget.exhausted true means an honest partial; the next run continues from the least-recently-checked product. All titles/links/dates verbatim from the vendor’s own feed or page.',
