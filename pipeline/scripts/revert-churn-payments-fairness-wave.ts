// Surgical, auditable revert of judge re-roll churn introduced by the payments fairness wave
// (2026-09-14: payments/adyen + paypal + square — the integrity counterpart to the Stripe
// agentic-capability spike). Same mechanics and ruling as revert-churn-stripe-agentic-wave.ts:
// a re-judged cell whose verdict or quality changed while citing NO evidence id that is new to
...
//
// Wave specifics:
//  - adyen/paypal/square were re-judged because this wave expanded their evidence packs
//    (11/9/10 new agent-surface urls.extra crawled + re-extract, 11/10/7 verbatim supplement
//    items, 1/2/2 live probes — see append-payments-fairness-agentic-evidence.py). Unlike the
//    Stripe wave there is NO stale-cache complication: spike-check-payments-baseline.ts
//    verified all payments judge caches hash-fresh against their HEAD packs before this wave
//    touched them, so the "new evidence" baseline for all three products is plain HEAD (the
//    commit this wave's worktree branched from).
//  - No payments golden cells exist at authoring time; the golden guard still runs (no-op).
//
// Usage: pnpm exec tsx pipeline/scripts/revert-churn-payments-fairness-wave.ts [--write]
...
// evidence" baseline for the no-new-citation rule). All fresh at HEAD for this wave.
const WAVE: Record<string, Record<string, string>> = {
  payments: { adyen: 'HEAD', paypal: 'HEAD', square: 'HEAD' },
}
