...
// #1 and #2 leaderboard products are within CLOSE_RACE_THRESHOLD PA Score points of each other, the
...
async function main(): Promise<void> {
  ...
  for (const cat of categories) {
    ...
    console.log(`\nuncertainty: ${cat.id} QUALIFIES — ${top1.productId} vs ${top2.productId} (Δ${diff} PA Score points)`)
    ...
    // groups that feed the PA Score's agent-readiness components (agent-access -> agentReady,
