// Minimal, hand-maintained mirror of the shapes in ../../lib/schemas.ts — the same convention
// as mcp/src/types.ts. Deliberately not imported from the Next app (this package is
// standalone/publishable and fetches JSON over HTTP rather than reading the repo's data/
// directly) — keep in sync by hand if lib/schemas.ts changes shape.
...
// The /api/scan quick-scan response (see components/SubmitScan.tsx and
// infra/cloudflare-proxy/worker.js in the main repo).
