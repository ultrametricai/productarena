...
3. **`POST /productarena/api/mcp-probe`** — the product pages' "Try it" live MCP handshake
   (`{arena, product}` body). Sends one JSON-RPC `initialize` (+ keyless `tools/list`) to the
   product's own documented remote MCP endpoint and returns a sanitized summary; a 401 with
   OAuth metadata is itself the result. Targets come ONLY from the hardcoded `MCP_ENDPOINTS`
   allowlist (generated by `scripts/generate-mcp-allowlist.mjs`, mirrored in
