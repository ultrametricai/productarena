...
export function renderArenaMarkdown(data: CategoryData, siteUrl: string): string {
  ...
  lines.push('')
  lines.push('| Rank | Product | PA Score | Score | Agentreadyness | API quality | MCP | CLI | API |')
  lines.push('|---|---|---|---|---|---|---|---|---|')
...
export function renderProductMarkdown(data: CategoryData, productId: string, siteUrl: string): string {
  ...
  lines.push('')
  lines.push(
    `Rank #${rank} · PA Score ${fmtNum(entry.aiEra)} · Score ${fmtNum(entry.score)} · Agentreadyness ${fmtNum(entry.agentReady)} · Agentic-app ${fmtNum(entry.agenticApp)} · API quality ${fmtNum(entry.apiQuality)} · ${entry.applicable}/${entry.total} stories applicable`,
  )
  lines.push('')
