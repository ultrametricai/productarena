...
describe('sortArenaRows', () => {
  ...
  it('sorts the claimsIntegrity column descending by default, nulls last', () => {
    const sorted = sortArenaRows(rows, 'claimsIntegrity', 'desc')
    expect(sorted.map((r) => r.productId)).toEqual(['c', 'b', 'a'])
  })

  it('sorts the claimsIntegrity column ascending, nulls still last', () => {
    const sorted = sortArenaRows(rows, 'claimsIntegrity', 'asc')
    expect(sorted.map((r) => r.productId)).toEqual(['b', 'c', 'a'])
  })
...
describe('defaultDirectionFor', () => {
  it('defaults numeric columns to descending', () => {
    ...
    expect(defaultDirectionFor('claimsIntegrity')).toBe('desc')
  })
