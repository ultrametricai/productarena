...
// InitClient and returns plain data; mcp/src/server.ts just wraps these in
// registerTool() calls and formats the result as tool content.
import type { InitClient } from './client.js'
import type { Battle, Category, Evidence, Product, Rankings, Story, Verdict } from './types.js'

export class InitError extends Error {}

async function fetchCategories(client: InitClient): Promise<Category[]> {
  return client.fetchJson<Category[]>('/data/categories.json')
}

async function assertKnownCategory(client: InitClient, category: string): Promise<void> {
  const categories = await fetchCategories(client)
  if (!categories.some((c) => c.id === category)) {
    throw new InitError(`unknown category "${category}" — see list_arenas for valid ids`)
  }
}

export async function listArenas(client: InitClient): Promise<Category[]> {
  return fetchCategories(client)
}

export async function getRankings(client: InitClient, category: string): Promise<Rankings> {
...
export async function getProduct(client: InitClient, category: string, productId: string): Promise<ProductDetail> {
  await assertKnownCategory(client, category)
  ...
  if (!product) {
    throw new InitError(`unknown product "${productId}" in category "${category}"`)
  }
...
export async function getBattle(client: InitClient, category: string, a: string, b: string): Promise<Battle> {
  await assertKnownCategory(client, category)
  ...
  if (!battle) {
    throw new InitError(`no battle found between "${a}" and "${b}" in category "${category}"`)
  }
  return battle
}
...
export async function searchProducts(client: InitClient, query: string): Promise<SearchResult[]> {
...
export async function getStoryVerdicts(client: InitClient, category: string, storyId: string): Promise<StoryVerdictsResult> {
  await assertKnownCategory(client, category)
  ...
  if (!story) {
    throw new InitError(`unknown story "${storyId}" in category "${category}"`)
  }
